...
impl Builtin for CompleteCmd {
    ...
    fn run(&self, shell: &mut Shell, argv: &[String]) -> BuiltinResult {
        ...
        let (mut root_opts, mut root_subs) = match fetch_best(&[cmd.as_str()], &mut warnings) {
        ...
        if cmd == "git" && root_subs.len() < 10 {
            match fetch_git_subcommands() {
                ...
                Ok(_) => {}
                Err(e) => warnings.push(format!("(warn) failed to enumerate git subcommands: {e}\n")),
            }
        }
        ...
        // 保存（トップレベル）
        {
            ...
            for o in &root_opts { centry.options.insert(o.clone()); }
            for s in &root_subs { centry.subcommands.entry(s.clone()).or_default(); }
        }
        ...
        for sub in &root_subs {
            ...
            if let Some((sub_opts, _subsubs, _src)) = fetch_best(&segs, &mut warnings) {
                ...
                for o in &sub_opts { sentry.options.insert(o.clone()); }
            }
        }
        ...
        BuiltinResult {
            ...
            stderr: if warnings.is_empty() { String::new() } else { warnings.join("\n") },
            code: 0,
        }
    }
}
...
fn fetch_git_subcommands() -> std::io::Result<BTreeSet<String>> {
    ...
    if let Ok(out) = Command::new("git")
        ...
        .env("GIT_PAGER", "cat").env("PAGER", "cat").env("LESS", "FRX").env("TERM", "dumb")
        .stdout(Stdio::piped()).stderr(Stdio::piped())
        .output()
    {
        let txt = merge_out(&out.stdout, &out.stderr);
...
fn fetch_best(segments: &[&str], warnings: &mut Vec<String>) -> Option<(BTreeSet<String>, BTreeSet<String>, &'static str)> {
    // 1) man → 抽出
    match get_man_text(segments) {
        Ok(Some(text)) => {
            let o = extract_options_from_text(&text);
            let s = extract_subcommands(&text);
            if !o.is_empty() || !s.is_empty() { return Some((o, s, "man")); }
        }
        Ok(None) => {}
        Err(e) => warnings.push(format!("(warn) man failed for `{}`: {e}", segments.join(" "))),
    }
    ...
    match get_help_text(segments) {
        Ok(Some(text)) => {
            let o = extract_options_from_text(&text);
            let s = extract_subcommands(&text);
            if !o.is_empty() || !s.is_empty() { return Some((o, s, "help")); }
        }
        Ok(None) => {}
        Err(e) => warnings.push(format!("(warn) --help failed for `{}`: {e}", segments.join(" "))),
    }
    None
}
...
fn get_man_text(segments: &[&str]) -> std::io::Result<Option<String>> {
    ...
    if segments.len() >= 2 { candidates.push(segments.join("-")); }
    candidates.push(segments[0].to_string());

    for page in candidates {
        let out = Command::new("man").args(["-P", "cat", &page])
            .env("MANPAGER", "cat")
            .stdout(Stdio::piped()).stderr(Stdio::piped())
            .output();
        match out {
...
fn get_help_text(segments: &[&str]) -> std::io::Result<Option<String>> {
    ...
    match out {
        Ok(o) => {
            let txt = merge_out(&o.stdout, &o.stderr);
            if txt.trim().is_empty() { Ok(None) } else { Ok(Some(txt)) }
        }
        Err(e) => Err(e),
    }
}
...
fn strip_overstrike_and_controls(s: &str) -> String {
    ...
    for ch in s.chars() {
        match ch { '\u{08}' => { buf.pop(); }, '\u{0C}' | '\u{00}' => {}, _ => buf.push(ch) }
    }
    ...
    while i < buf.len() {
        ...
        if c == '\u{1b}' {
            ...
            if i < buf.len() && buf[i] == '[' {
                i += 1;
                while i < buf.len() { let cc = buf[i]; if cc.is_ascii_alphabetic() { i += 1; break; } i += 1; }
                continue;
            }
            continue;
        }
...
pub fn extract_options_from_text(text: &str) -> BTreeSet<String> {
    ...
    for raw in text.split_whitespace() {
        ...
        if let Some(rest) = token.strip_prefix("--") {
            if !rest.chars().next().map(|c| c.is_ascii_alphanumeric()).unwrap_or(false) { continue; }
            let name = rest.split(&['=', ' '][..]).next().unwrap_or(rest);
            let clean = name.chars().take_while(|c| c.is_ascii_alphanumeric() || *c == '-').collect::<String>();
            if !clean.is_empty() { set.insert(format!("--{}", clean)); }
            continue;
        }
        if let Some(rest) = token.strip_prefix('-') {
            if rest.is_empty() || rest.starts_with('-') { continue; }
            for ch in rest.chars() {
                if ch.is_ascii_alphabetic() { set.insert(format!("-{}", ch)); } else { break; }
            }
        }
    }
    set
...
pub fn extract_subcommands(text: &str) -> BTreeSet<String> {
    ...
    let headings = [
        "SUBCOMMANDS:", "Subcommands:", "Sub-Commands:",
    ...
    while i < lines.len() {
        ...
        if headings.iter().any(|h| line.starts_with(h)) {
            ...
            while i < lines.len() {
                ...
                // 次のセクション見出しらしき行で終了
                if (l.trim_end().ends_with(":") && l == l.trim()) || l.chars().all(|c| c.is_ascii_uppercase() || c.is_whitespace()) { break; }
                let token = l.trim_start().split_whitespace().next().unwrap_or("");
                let token = token.trim_matches(|c: char| matches!(c, ',' | ';'));
                let clean: String = token.chars().take_while(|c| c.is_ascii_alphanumeric() || *c == '-' || *c == '_').collect();
                if !clean.is_empty() { subs.insert(clean); }
                i += 1;
            }
            continue;
        }
